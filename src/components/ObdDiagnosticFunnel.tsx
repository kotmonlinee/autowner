...
export default function ObdDiagnosticFunnel({ code, symptoms, diagnosticCauses }: Props) {
  ...
  // Rank causes: default by probability, by match score when symptoms selected
  const rankedCauses = useMemo(() => {
    if (diagnosticCauses.length === 0) return [];

    if (selectedSymptoms.size === 0) {
      return [...diagnosticCauses].sort((a, b) => b.probability - a.probability);
    }
    ...
    return scored.sort((a, b) => b.score - a.score || b.probability - a.probability);
  }, [selectedSymptoms, diagnosticCauses]);

  const hasSelection = selectedSymptoms.size > 0;
    ...
    <div className="bg-surface-1 rounded-xl border-2 border-primary/15 p-5 mb-4">
          ...
          {symptoms.map((s) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2.5 rounded-lg text-xs font-heading font-semibold border transition-all cursor-pointer min-h-[44px] ${
                  isSelected
                    ? "bg-primary text-white border-primary shadow-sm shadow-primary/20"
                    : "bg-surface-0 border-surface-border text-text-secondary hover:border-primary/50 hover:bg-primary/5 active:scale-95"
                }`}
              >
                <span className="flex items-center gap-1.5">
                  <span className={`text-xs transition-opacity ${isSelected ? "opacity-100" : "opacity-40"}`}>
                    {isSelected ? "✓" : "+"}
                  </span>
                  {s}
                </span>
      ...
      {rankedCauses.length > 0 && (
        <div className="border-t border-surface-border pt-4">
          ...
          <p className="text-xs text-text-muted mb-3">
            {hasSelection
              ? `Based on your ${selectedSymptoms.size} selected symptom${selectedSymptoms.size > 1 ? "s" : ""}. Matching causes ranked first:`
              : "Ranked by probability. Select symptoms above to refine the order."}
          </p>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 border-t border-surface-border pt-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-surface-border">
                        ...
                        <Link
                          href={cause.repairSlug ? `/repair-cost/${cause.repairSlug}` : "/repair-cost"}
      ...
      {confirmedCause && (
        <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-950/20 border-2 border-emerald-300 dark:border-emerald-700 rounded-xl">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href={confirmedCause.repairSlug ? `/repair-cost/${confirmedCause.repairSlug}` : "/repair-cost"}
