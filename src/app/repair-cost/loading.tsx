export default function RepairCostLoading() {
  return (
    <div className="min-h-screen bg-surface-0 flex flex-col">
      ...
      <main className="max-w-4xl mx-auto px-5 py-6 flex-1 w-full animate-pulse">
        ...
        <div className="h-8 w-80 rounded bg-surface-2 mb-8" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="h-32 rounded-xl bg-surface-1 border border-surface-border" />
          ))}
        </div>
      </main>
    </div>
